...
class getViewZone {
  ...
  generateViewZone = (sta) => {
    ...
    for (let reel = 0; reel < column; reel++) {
      ...
      if (wildCounter > 1) {
        let newSymbolArray = this.countOfWild(
          arrayOfReel[reel],
          arrayOfReel[reel].length,
          reel,
          row
  ...
  checkPayline = async (payArray, matrixReel, pay, req, res) => {
    ...
    let winAmount = 0;
    let winFreeSpinAmount = (await userHelper.gameData(req, res))
      .winFreeSpinAmount;
      ...
      for (
        ...
        if (payLine) {
          ...
          if (count > 2) {
            ...
            let symbolOfResult = this.buildPayLine(
              count,
              symbol,
              pay,
              payLine,
  ...
  countOfSymbol = (matrixReel, payLine, rowOfMatrix) => {
    ...
    if (payLine[0] === rowOfMatrix) {
      ...
      for (let element = 1; element < payLine.length; element++) {
        ...
        if (
          matrixReel[payLine[element]][element] !== "WILD" &&
          matrixReel[payLine[element]][element] !== symbol
        ) {
          break;
        }
        count++;
  ...
  buildPayLine = (
    ...
    if (freeSpin > 0) {
      winFreeSpinAmount = this.creditWinAmount(
        multipliar,
        betAmount,
        winFreeSpinAmount
      );
    ...
    return {
      ...
      payLine,
      winAmount: betAmount + multipliar,
      winFreeSpinAmount,
    };
  };
